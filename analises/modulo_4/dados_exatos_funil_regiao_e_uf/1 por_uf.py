# %% Funil FIES - Por UF (com tabela exibida antes de cada gráfico)
import pandas as pd
import matplotlib.pyplot as plt
import numpy as np
import IPython.display as display  # para exibir tabelas no notebook

# --- 1. Ler arquivo processado por UF ---
...
largura_barra = 0.12
cores_ufs = plt.cm.tab20(np.linspace(0, 1, len(ufs)))  # paleta grande e distinta

# --- 5. Loop por área CINE ---
for area in areas:
    df_area = df[df['nome_cine_area_geral'] == area]

    # Agregar dados por período e UF
    df_area_grouped = (
    ...
    ax.legend(title='UF', bbox_to_anchor=(1.02, 1), loc='upper left', ncol=2, fontsize=7)
...
ax.set_xticklabels([nomes_curto[c] for c in colunas_funil], rotation=30, ha='right')
ax.set_ylabel('Quantidade')
ax.set_title('Funil FIES – Nacional (Todas as Áreas, por UF)', fontsize=14, fontweight='bold')
ax.grid(axis='y', linestyle='--', alpha=0.4)
ax.legend(title='UF', bbox_to_anchor=(1.02, 1), loc='upper left', ncol=2, fontsize=7)

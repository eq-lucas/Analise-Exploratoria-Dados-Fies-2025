...
# Caminho para os arquivos JÁ MESCLADOS (com NaNs)
path_leitura = '../../../planilhas/limpo/modulo_1/inscricao_coluna_CINE/'

# Coluna que queremos CHECAR se é NaN
coluna_nan = 'NO_CINE_AREA_GERAL' 

# Coluna de NOME do curso que queremos AGRUPAR
coluna_nome_curso_fies = 'nome_curso_inscricao' 

# --- 2. Ler e Concatenar todos os arquivos ---
print(f"Lendo arquivos de: {path_leitura}")
lista_dfs = []
try:
    lista_nomes_arquivos = os.listdir(path_leitura)
except FileNotFoundError:
    print(f"ERRO: Diretório não encontrado: {path_leitura}")
...
if not lista_dfs:
    print("Nenhum dataframe foi lido. Análise encerrada.")
else:
    # Junta todos os arquivos (2019, 2020, 2021) em um só
    df_agrupado = pd.concat(lista_dfs, ignore_index=True)
    print("Todos os arquivos foram concatenados.")
    ...
    if total_nans > 0:
        print(f"Total de NaNs encontrados em todos os arquivos: {total_nans}")
        ...
        # 3. Mostra o resultado (é uma Série do Pandas, o "df" que você pediu)
        display(contagem_nomes_nan)
        
        # Opcional: Salvar esta análise em um CSV para você ver melhor
        caminho_salvar_analise = '../../../planilhas/limpo/modulo_2/ajustar_nan_cine_inscritos/analise_nomes_nan_inscricao.csv'

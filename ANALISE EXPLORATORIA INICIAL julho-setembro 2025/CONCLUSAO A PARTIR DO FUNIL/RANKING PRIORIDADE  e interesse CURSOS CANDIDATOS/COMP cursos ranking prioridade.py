...
if df_geral is not None:
    ...
    print("\n--- Passo 1: Agregando o total de 'Candidatos_Unicos_Validados' por curso e período...")
    # Agrupa e soma apenas a coluna de interesse: Candidatos_Unicos_Validados
    df_agregado = df_geral.groupby(['Periodo', 'Nome do Curso'])['Candidatos_Unicos_Validados'].sum().reset_index()
    ...
    df_agregado['Posicao_Ranking_Geral'] = df_agregado.groupby('Periodo')['Candidatos_Unicos_Validados'].rank(method='dense', ascending=False).astype(int)
    ...
    ax.set_title('Evolução da Posição dos Cursos de Computação no Ranking de Prioridade (por Candidatos)', fontsize=16, pad=20, weight='bold')

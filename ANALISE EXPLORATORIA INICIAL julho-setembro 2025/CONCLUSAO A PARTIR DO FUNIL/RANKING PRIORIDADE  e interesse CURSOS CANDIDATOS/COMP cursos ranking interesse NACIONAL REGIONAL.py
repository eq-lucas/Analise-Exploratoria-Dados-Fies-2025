...
def calcular_distribuicao_interesse(df, groupby_cols):
    ...
    df_agregado = df.groupby(groupby_cols + ['Nome do Curso'])['Candidatos_Unicos_Validados'].sum().reset_index()
    
    # Calcula o TOTAL de candidatos em computação para o grupo principal (nacional ou regional)
    df_agregado['Total_Candidatos_Comp'] = df_agregado.groupby(groupby_cols)['Candidatos_Unicos_Validados'].transform('sum')
    
    # Calcula a Distribuição de Interesse (Market Share)
    df_agregado = df_agregado[df_agregado['Total_Candidatos_Comp'] > 0]
    df_agregado['Distribuicao_Interesse'] = df_agregado['Candidatos_Unicos_Validados'] / df_agregado['Total_Candidatos_Comp']
...
if df_comp is not None:
    ...
    titulo_nac = 'Distribuição de Interesse de Candidatos entre Cursos de Computação (Nacional)'
    ...
    for regiao in df_regional['Regiao'].dropna().unique():
        ...
        titulo_reg = f'Distribuição de Interesse de Candidatos em Computação - Região {regiao}'

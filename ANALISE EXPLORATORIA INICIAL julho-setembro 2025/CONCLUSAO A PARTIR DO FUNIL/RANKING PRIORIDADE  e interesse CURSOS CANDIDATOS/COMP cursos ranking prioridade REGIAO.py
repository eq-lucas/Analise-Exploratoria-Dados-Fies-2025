...
# O ranking agora é baseado PURAMENTE na contagem de 'Candidatos_Unicos_Validados'
...
if df_geral is not None:
    print("\n--- Passo 1: Agregando o total de 'Candidatos_Unicos_Validados' por curso, região e período...")
    
    # Agrupa e soma apenas a coluna de interesse: Candidatos_Unicos_Validados
    df_agregado = df_geral.groupby(['Regiao', 'Periodo', 'Nome do Curso'])['Candidatos_Unicos_Validados'].sum().reset_index()
    
    print("--- Passo 2: Criando o ranking baseado no volume de candidatos DENTRO de cada Região e Período...")
    # O ranking agora é feito diretamente sobre a contagem de 'Candidatos_Unicos_Validados'
    df_agregado['Posicao_Ranking_Regional'] = df_agregado.groupby(['Regiao', 'Periodo'])['Candidatos_Unicos_Validados'].rank(method='dense', ascending=False).astype(int)
    ...
    for regiao in regioes:
        ...
        ax.set_title(f'Evolução da Posição no Ranking de Prioridade (por Candidatos) - REGIÃO {regiao.upper()}', fontsize=16, pad=20, weight='bold')

...
if df_geral is not None:
    ...
    agregado_comp.rename(columns={'Inscritos_Validados': 'Candidatos_em_Comp'}, inplace=True)

    agregado_outros = df2_outros.groupby('Periodo')['Inscritos_Validados'].sum().reset_index()
    agregado_outros.rename(columns={'Inscritos_Validados': 'Candidatos_em_Outros'}, inplace=True)
    ...
    caminho_csv = os.path.join(pasta_planilhas, 'analise_interesse_inscritos_comp_vs_outros.csv')
    ...
    ax.set_title('Distribuição de Interesse dos inscritos Focados em Computação', fontsize=18, pad=20, weight='bold')
    ...
    caminho_grafico = os.path.join(output_folder_graficos, 'distribuicao_interesse_inscritos.png')

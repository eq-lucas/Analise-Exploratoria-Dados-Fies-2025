...
# =============================================================================
# MENTOR'S NOTES:
# Este script adapta a "Análise de Interesse" para ser executada POR REGIÃO.
# Ele itera sobre cada região, calcula a distribuição de inscritos
# (Comp vs. Outros) e gera um gráfico para cada uma.
# =============================================================================

# --- 1. SETUP E CARGA DOS DADOS ---
print("Carregando o DataFrame GERAL para a análise de interesse por região...")
try:
...
if df_geral is not None:
    # Lista para armazenar os resultados de cada região
    lista_resultados_regionais = []
    
    # Pasta de saída para os gráficos
    output_folder_graficos = os.path.join('GRAFICOS', 'COMP regiao', 'ANALISE DE INTERESSE (INSCRITOS)')
    os.makedirs(output_folder_graficos, exist_ok=True)
    
    # Pega a lista de todas as regiões presentes nos dados
    ...
    for regiao in regioes:
        ...
        # Filtra o DataFrame principal para a região atual
        ...
        agregado_comp = df1_computacao.groupby('Periodo')['Inscritos_Validados'].sum().reset_index()
        agregado_comp.rename(columns={'Inscritos_Validados': 'Inscritos_em_Comp'}, inplace=True)

        agregado_outros = df2_outros.groupby('Periodo')['Inscritos_Validados'].sum().reset_index()
        agregado_outros.rename(columns={'Inscritos_Validados': 'Inscritos_em_Outros'}, inplace=True)

        df_resultado_regiao = pd.merge(agregado_comp, agregado_outros, on='Periodo', how='outer').fillna(0)
        
        df_resultado_regiao['Total_Inscritos'] = df_resultado_regiao['Inscritos_em_Comp'] + df_resultado_regiao['Inscritos_em_Outros']
        df_resultado_regiao = df_resultado_regiao[df_resultado_regiao['Total_Inscritos'] > 0]
        
        if df_resultado_regiao.empty:
            print(f"  -> AVISO: Não há dados de inscritos para a região {regiao}. Gráfico não será gerado.")
            continue

        df_resultado_regiao['Percentual_Comp'] = (df_resultado_regiao['Inscritos_em_Comp'] / df_resultado_regiao['Total_Inscritos'])
        df_resultado_regiao['Percentual_Outros'] = (df_resultado_regiao['Inscritos_em_Outros'] / df_resultado_regiao['Total_Inscritos'])
        
        # Adiciona a coluna de Região para o CSV final
        ...
        ax.set_title(f'Distribuição de Interesse dos Inscritos - Região {regiao.upper()}', fontsize=18, pad=20, weight='bold')
        ax.set_xlabel('Período', fontsize=14)
        ax.set_ylabel('Percentual de Inscritos', fontsize=14)
        ...
        caminho_grafico = os.path.join(output_folder_graficos, f'distribuicao_interesse_inscritos_regiao_{slugify(regiao)}.png')
    ...
    if lista_resultados_regionais:
        ...
        # Reorganiza colunas
        colunas_ordenadas = ['Regiao', 'Periodo', 'Inscritos_em_Comp', 'Inscritos_em_Outros', 'Total_Inscritos', 'Percentual_Comp', 'Percentual_Outros']
        df_final_todas_regioes = df_final_todas_regioes[colunas_ordenadas]

        pasta_planilhas = '../../../planilhas'
        os.makedirs(pasta_planilhas, exist_ok=True)
        caminho_csv = os.path.join(pasta_planilhas, 'analise_interesse_inscritos_POR_REGIAO.csv')
        ...
        print("\n\nAnálise de interesse por região concluída com sucesso!")
    else:

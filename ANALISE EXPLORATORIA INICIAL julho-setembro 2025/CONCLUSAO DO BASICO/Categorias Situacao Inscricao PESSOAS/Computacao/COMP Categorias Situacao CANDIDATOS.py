...
for year in ano: 
    for semestre in sem:
        ...
        filtro= (df_bruto['Nome do curso'].isin(cursosComputacao)) 

        df_filtrado=(df_bruto[filtro].copy()) 
        ...
        candidatos_df=(df_filtrado.sort_values(by=OrdemCorretaCandidatosUnicos, ascending=True)
                     .drop_duplicates(subset='ID do estudante',keep='first')
                     .sort_values(by=ordem_inscricoes,ascending=True))




        df_agrupar_cursos= (candidatos_df.
...
# %%
salvar_caminho= '../../planilhas basico/COMP Categorias Situacao CANDIDATOS.csv'

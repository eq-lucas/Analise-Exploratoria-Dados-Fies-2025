...
for year in ano: 
    for semestre in sem:
        ...
        ins_df_sort= (ins_df.sort_values(by=ordem_inscricoes,ascending=[True,True,True,True]))
...
c['TOTAL INSCRICOES']= c[colunas].apply(sum,axis=1)
...
c.loc['total',['ano','semestre','TOTAL INSCRICOES']]= np.nan
...
# %%
c.to_csv('../planilhas basico/Categorias Situacao Inscricao GERAL.csv')


# %%
z=pd.read_csv('../planilhas basico/Categorias Situacao Inscricao GERAL.csv',index_col='Unnamed: 0')
z
# %%
